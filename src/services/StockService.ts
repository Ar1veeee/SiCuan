...
export const getStockSummaryService = async (
  userId: string
...
export const createStockTransactionService = async (
  ...
  if (transactionData.jenis_transaksi === "PENJUALAN") {
    if (bahan.jumlah < transactionData.jumlah) {
      throw new ApiError(
        `Stok tidak cukup. Stok ${bahan.nama_bahan} saat ini: ${bahan.jumlah}`,
        400
      );
    }
  }

  const existingBahan = await StockModel.findBahanByName(
    userId,
    bahan.nama_bahan
  );
...
export const deleteStockService = async (
  ...
  const hppUpdatePromises = affectedMenuIds.map((menuId) =>
    updateTotalHPPService(menuId)
  );
