...
const StockModel = {
  ...
  createTransactionAndUpdateBahan: async (
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (data.jenis_transaksi === "PENYESUAIAN") {
        const bahanSaatIni = await tx.bahan.findUniqueOrThrow({
          where: { id: bahanId },
        });
  ...
  findBahanByUserId: async (userId: string) => {
    return await prisma.bahan.findMany({
      where: { userId },
    });
  },

  findBahanByName: async (userId: string, nama_bahan: string) => {
  ...
  findBahanByIdAndUserId: async (userId: string, bahanId: string) => {
    return await prisma.bahan.findFirst({
      where: {
        id: bahanId,
        userId,
   ...
   */
  deleteBahanAndRelations: async (bahanId: string) => {
    return prisma.$transaction(async (tx) => {
