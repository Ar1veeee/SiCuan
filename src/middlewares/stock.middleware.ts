...
export const verifyAndAttachBahan = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.userId || !req.bahanId) {
            throw ApiError.badRequest("ID User atau Bahan tidak tersedia.");
        }
        
        const bahan = await StockModel.findBahanByIdAndUserId(req.userId, req.bahanId);
        if (!bahan) {
            throw ApiError.notFound("Bahan tidak ditemukan atau Anda tidak memiliki akses.");
        }
